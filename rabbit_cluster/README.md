# Домашнее задание к занятию  «Очереди RabbitMQ» - Серегин А.Ю.
...
Количество нод регулируется в файле vagrant. В данный момент установлено (1..2), если изменим второе значение например на 240 - поднимется 239 нод. 
Чтобы не усложнять процесс распространения cookie файла RabbitMQ я удалил его на каждой ноде и создал свой "кастомный" (для сервера важно чтобы он был одинаковый, при этом все равно, какое значение там будет прописано). 
